...
(() => {
  ...
  // src/tools/export.ts
  function serializeNodeDetailed(node, depth, maxDepth) {
    var base = {
      id: node.id,
      name: node.name,
      type: node.type
    ...
    if ("fills" in node) {
      ...
      if (fills !== figma.mixed && Array.isArray(fills)) {
        base.fills = fills.map(function(fill) {
          if (fill.type === "SOLID") {
            return {
              type: "SOLID",
              color: { r: fill.color.r, g: fill.color.g, b: fill.color.b },
    ...
    if ("strokes" in node) {
      ...
      if (Array.isArray(strokes) && strokes.length > 0) {
        base.strokes = strokes.map(function(stroke) {
          if (stroke.type === "SOLID") {
            return {
              type: "SOLID",
              color: { r: stroke.color.r, g: stroke.color.g, b: stroke.color.b },
  ...
  async function exportFrameImage(args) {
    var node = figma.getNodeById(args.frame_id);
    if (!node) {
      return { error: "Node not found with ID: " + args.frame_id };
    }
